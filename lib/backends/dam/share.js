...
DAMShare.prototype.buildContentPath = function (path) {
return this.buildUrlRoot() + DAM.DAM_ROOT_PATH + encodeURI(utils.normalizeSMBFileName(utils.stripParentPath(this.path, this.contextPath + DAM.DAM_ROOT_PATH) + path));
};

DAMShare.prototype.buildCreateAssetPath = function (path) {
  var assetPath = this.buildContentPath(path);
...
DAMShare.prototype.getDeleteResourceError = function (path, resp) {
  if (resp.statusCode === 404) {
    return new SMBError(ntstatus.STATUS_NO_SUCH_FILE, 'cannot delete file because it was not found ' + path);
  } else if (resp.statusCode === 423) {
    return new SMBError(ntstatus.STATUS_ACCESS_DENIED, 'cannot delete file due to 423 response code ' + path);
  } else if (resp.statusCode !== 200) {
    return new SMBError(ntstatus.STATUS_UNSUCCESSFUL, 'cannot delete file due to ' + resp.statusCode + ' response code ' + path);
  }
  return false;
};
  ...
  fs.stat(localPath, function (err, stats) {
    ...
    async.whilst(function () { return chunkOffset < totalSize; }, function (whileCb) {
      ...
      req.on('abort', function () {
        ...
        whileCb({message: 'upload of path ' + remotePath + ' was aborted', ignoreEmit: true});
      });
      var form = req.form();
      if(replace) {
...
DAMShare.prototype.deleteResource = function (tree, path, isFile, cb) {
  if (!checkDotFile.call(this, tree, path, 'DELETE', cb)) {
    JCRShare.prototype.deleteResource.call(this, tree, path, isFile, cb);
  }
};
