  ...
  emptyStream.pipe(
    webutils.submitRequest(options, function (err, resp, body) {
      if (err) {
        ...
        logger.error('failed to create %s - %s %s [%d]', name, resp.method, resp.href, resp.statusCode, body);
...
DAMTree.prototype.createDirectory = function (name, cb) {
  ...
  var req = webutils.submitRequest(options, function (err, resp, body) {
    if (err) {
      ...
      logger.error('failed to create %s - %s %s [%d]', name, 'POST', url, resp.statusCode, body);
...
DAMTree.prototype.deleteDirectory = function (name, cb) {
  ...
  options.headers[JCR.ACTION_HEADER] = JCR.ACTION_DELETEFOLDER;
