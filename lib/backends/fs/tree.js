...
var FSFileConnection = require('./fileconnection');
 ...
 */
var FSTree = function (treeConnection, context) {
  if (!(this instanceof FSTree)) {
    return new FSTree(treeConnection, context);
  }
 ...
 */
FSTree.prototype.getPerfLogger = function () {
  return this.context.perf();
};

/**
 * Create a new File instance for use by the tree.
 * @param name The name of the file to create.
 * @param {Function} cb callback called on completion
 * @param {SMBError} cb.error error (non-null if an error occurred)
 * @param {File} cb.file File instance
 */
FSTree.prototype.createFileInstance = function (name, cb) {
  FSFileConnection.createFileInstance(name, this, cb);
};
 ...
 */
FSTree.prototype._exists = function (name, cb) {
 ...
 */
FSTree.prototype._open = function (name, cb) {
  var logger = this.getLogger();
 ...
 */
FSTree.prototype._list = function (pattern, cb) {
  var logger = this.getLogger();
  var perflog = this.getPerfLogger();
 ...
 */
FSTree.prototype._createFile = function (name, cb) {
  var logger = this.getLogger();
  var perflog = this.getPerfLogger();
 ...
 */
FSTree.prototype._createDirectory = function (name, cb) {
  var logger = this.getLogger();
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
FSTree.prototype._delete = function (name, cb) {
  var logger = this.getLogger();
  var perflog = this.getPerfLogger();
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
FSTree.prototype._deleteDirectory = function (name, cb) {
  var logger = this.getLogger();
  var perflog = this.getPerfLogger();
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
FSTree.prototype._rename = function (oldName, newName, cb) {
  var logger = this.getLogger();
  var perflog = this.getPerfLogger();
