...
var IPCFileConnection = require('./fileconnection');
 ...
 */
var IPCTree = function (treeConnection, context) {
  if (!(this instanceof IPCTree)) {
    return new IPCTree(treeConnection, context);
  }
...
IPCTree.prototype.open = function (name, cb) {
  ...
  var self = this;
  if (this.pipes[name]) {
    process.nextTick(function () { cb(null, new IPCFile(new IPCFileConnection(name, self), self)); });
  } else {
...
IPCTree.prototype.list = function (pattern, cb) {
  ...
  var files = _.keys(this.pipes).map(function (nm) {
    return new IPCFile(new IPCFileConnection(nm, self), self);
  });
