 ...
 * @private
 * @this {JCRFile}
 * @param {JCRFileConnection} fileConnection existing file connection
 * @param {JCRTree} tree tree object
 */
var JCRFile = function (fileConnection, tree) {
  if (!(this instanceof JCRFile)) {
    return new JCRFile(fileConnection, tree);
  }
...
JCRFile.prototype.ensureGotLocalCopy = function (cb) {
  ...
  var self = this;
  this.tree.share.getLocalFile(this.tree, this.filePath, this.lastModified(), function (err, localFilePath) {
    if (err) {
      cb(err);
    } else {
...
JCRFile.prototype.syncLocalChanges = function (cb) {
  var logger = this.getLogger();
  if (!this.localFile || !this.fileConnection.isDirty()) {
  ...
  this.ensureGotLocalCopy(function (err, localFile) {
    ...
    self.tree.share.updateResource(self.tree, self.filePath, localFile.path, function (err, stats) {
      // update length and lastModified
      self.fileConnection.setFileLength(stats.size);
      self.setLastModified(stats.mtime.getTime());
      self.fileConnection.setDirty(false);
    });
  });
};
...
JCRFile.prototype.patchContent = function (content, fileLength, lastModified) {
  ...
  content[JCR.JCR_CONTENT][JCR.JCR_DATA_LENGTH] = this.fileConnection.getFileLength();
  // update lastModified content representation
  content[JCR.JCR_CONTENT][JCR.JCR_LASTMODIFIED] = new Date(lastModified).toISOString();
...
JCRFile.prototype.isFile = function () {
  return this.tree.share.isFilePrimaryType(this.fileConnection.getContent()[JCR.JCR_PRIMARYTYPE]);
};
...
JCRFile.prototype.isDirectory = function () {
  return this.tree.share.isDirectoryPrimaryType(this.fileConnection.getContent()[JCR.JCR_PRIMARYTYPE]);
};
...
JCRFile.prototype.isReadOnly = function () {
  return this.fileConnection.getContent()[JCR.JCR_ISCHECKEDOUT] === false;
};
...
JCRFile.prototype.size = function () {
  return this.fileConnection.getFileLength();
};
...
JCRFile.prototype.lastModified = function () {
  var content = this.fileConnection.getContent();
  if (this.isFile() && content[JCR.JCR_CONTENT][JCR.JCR_LASTMODIFIED]) {
    return new Date(content[JCR.JCR_CONTENT][JCR.JCR_LASTMODIFIED]).getTime();
  } else {
    return this.created();
  }
};
...
JCRFile.prototype.setLastModified = function (ms) {
  if (this.isFile()) {
    ...
    this.fileConnection.getContent()[JCR.JCR_CONTENT][JCR.JCR_LASTMODIFIED] = new Date(ms).toISOString();
  }
};
...
JCRFile.prototype.created = function () {
  return new Date(this.fileConnection.getContent()[JCR.JCR_CREATED]).getTime();
};
  ...
  function updateStats(localFile, callback) {
    fs.fstat(localFile.fd, function (err, stats) {
      if (err) {
        ...
        self.fileConnection.setFileLength(stats.size);
        self.setLastModified(stats.mtime.getTime());
        self.fileConnection.setDirty(true);
        // touch cache entry
        self.tree.share.touchLocalFile(self.tree, self.filePath, self.lastModified(), function (err) {
          callback(err);
        });
      }
    });
  ...
  this.ensureGotLocalCopy(function (err, localFile) {
    ...
    fs.ftruncate(localFile.fd, length, function (err) {
      if (err) {
        ...
        // update length and lastModified
        self.fileConnection.setFileLength(length);
        ...
        self.tree.share.getContent(self.tree, self.filePath, false, function (err, content) {
          if (content) {
...
JCRFile.prototype.delete = function (cb) {
  var self = this;
  this.getTree().delete(this.filePath, function (err) {
    if (err) {
      cb(err);
      return;
    }
...
JCRFile.prototype.close = function (cb) {
  ...
  function sync(callback) {
    if (self.fileConnection.isDirty()) {
