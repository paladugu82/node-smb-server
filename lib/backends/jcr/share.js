...
JCRShare.prototype.buildResourcePath = function (path) {
  return this.path + encodeURI(path);
};

JCRShare.prototype.getContent = function (tree, path, deep, cb) {
  var logger = wlogger;
...
JCRShare.prototype.emitSyncFileError = function (remotePath, method, err) {
  this.emitShareEvent('syncfileerr', getSyncFileData(remotePath, method, err));
};
...
JCRShare.prototype.updateResource = function (tree, remotePath, localPath, cb) {
  ...
  var method = 'PUT';
  localPath = this.unicodeNormalize(localPath);
    ...
    if (err) {
      ...
      var req = webutils.submitRequest(options, function (err, resp, body) {
        if (err) {
          ...
          sendCallback(SMBError.fromSystemError(err, 'unable to submit request due to unexpected error ' + remotePath));
          ...
          sendCallback(new SMBError(ntstatus.STATUS_UNSUCCESSFUL, 'unable to sync local changes due to unexpected status code ' + resp.statusCode + ' ' + remotePath));
        } else {
          // succeeded
          ...
          self.touchLocalFile(tree, remotePath, stats.mtime.getTime(), function (err) {
            sendCallback(err, stats);
          });
        }
      });
      req.on('abort', function () {
        logger.info('upload of path %s was aborted', remotePath);
        sendCallback({message: 'upload of path ' + remotePath + ' was aborted', ignoreEmit: true});
      });
...
JCRShare.prototype.getDeleteResourceError = function (path, resp) {
  if (resp.statusCode === 404) {
    return new SMBError(ntstatus.STATUS_NO_SUCH_FILE, 'unable to delete file because it does not exist ' + path);
  } else if (resp.statusCode !== 204) {
    return new SMBError(ntstatus.STATUS_UNSUCCESSFUL, 'unable to delete file due to ' + resp.statusCode + ' status code ' + path);
  }
  return false;
};
...
JCRShare.prototype.deleteResource = function (tree, path, isFile, cb) {
  ...
  return webutils.submitRequest(options, function (err, resp, body) {
    if (err) {
      ...
      cb(SMBError.fromSystemError(err, 'unable to delete file due to unexpected error ' + path));
    } else {
 ...
 * @param {Error} cb.err Truthy if there were errors.
 */
JCRShare.prototype.createFileResource = function (tree, remotePath, localPath, cb) {
  var logger = tree.getLogger();
  ...
  emptyStream.pipe(
    webutils.submitRequest(options, function (err, resp, body) {
      if (err) {
        ...
        cb(SMBError.fromSystemError(err, 'unable to create file due to unexpected error ' + remotePath));
      } else if (resp.statusCode === 409) {
        cb(new SMBError(ntstatus.STATUS_OBJECT_NAME_COLLISION, 'unable to create file due to 409 status code ' + remotePath));
      } else if (resp.statusCode !== 201) {
        logger.error('failed to create %s - %s %s [%d]', remotePath, options.method, options.url, resp.statusCode, body);
        cb(new SMBError(ntstatus.STATUS_UNSUCCESSFUL, 'unable to create file due to ' + resp.statusCode + ' status code ' + remotePath));
      } else {
        // succeeded
        // invalidate cache
...
JCRShare.prototype.createDirectoryResource = function (tree, path, cb) {
  ...
  return webutils.submitRequest(options, function (err, resp, body) {
    if (err) {
      ...
      cb(SMBError.fromSystemError(err, 'unable to create directory due to unexpected error ' + path));
    } else if (resp.statusCode === 409) {
      cb(new SMBError(ntstatus.STATUS_OBJECT_NAME_COLLISION, 'unable to create directory due to 409 status code ' + path));
    } else if (resp.statusCode !== 201) {
      logger.error('failed to create %s - %s %s [%d]', path, options.method, options.href, resp.statusCode, body);
      cb(new SMBError(ntstatus.STATUS_UNSUCCESSFUL, 'unable to create directory due to ' + resp.statusCode + ' status code ' + path));
    } else {
      // succeeded
      // invalidate cache
...
JCRShare.prototype.invalidateContentCache = function (tree, path, deep) {
  path = this.unicodeNormalize(path);
  var logger = tree.getLogger();
...
JCRShare.prototype.fetchResource = function (context, path, cb) {
  ...
  var req = webutils.submitRequest(options)
    ...
    .on('error', function (err) {
      self.unregisterDownloadRequest(path);
      self.emitShareEvent('downloaderr', {path: path, err: err});
      fs.unlink(tmpFilePath, function (ignored) {
        cb(err);
      });
    })
