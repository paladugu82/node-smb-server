...
var JCRFileConnection = require('./fileconnection');
 ...
 */
var JCRTree = function (treeConnection, context) {
  if (!(this instanceof JCRTree)) {
    return new JCRTree(treeConnection, context);
  }
...
JCRTree.prototype.createFileInstance = function (filePath, content, fileLength, cb) {
  ...
  if (_isDotFile(filePath)) {
    ...
    JCRFileConnection.createFileInstance(filePath, this, content, fileLength, cb);
  }
};
...
JCRTree.prototype.createFileInstancesFromContent = function (content, path, namePattern, cb) {
  ...
  this.share.parseContentChildEntries(content, function (childName, childContent) {
    ...
    if (namePattern === '*' || self.unicodeEquals(childName, namePattern)) {
      entries[self.unicodeNormalize(Path.join(path, childName))] = childContent;
    }
  });
...
JCRTree.prototype.fetchFileLength = function (path, cb) {
  ...
  opts.headers[JCR.ACTION_HEADER] = JCR.ACTION_FILELENGTH;
  var options = this.share.applyRequestDefaults(this, opts);
 ...
 */
JCRTree.prototype._exists = function (name, cb) {
  var logger = this.getLogger();
  ...
  opts.headers[JCR.ACTION_HEADER] = JCR.ACTION_EXISTS;
 ...
 */
JCRTree.prototype._open = function (name, cb) {
  var logger = this.getLogger();
 ...
 */
JCRTree.prototype._list = function (pattern, cb) {
  var logger = this.getLogger();
  ...
  function listRemoteFiles(tmpFiles, next) {
    ...
    function getContent(done) {
      self.share.getContent(self, parentPath, true, done);
    }
  ...
  if (fileName === '*') {
    ...
    async.waterfall([ listTempFiles, listRemoteFiles ], function (err, result) {
      if (!err) {
        ...
        self.share.emitShareEvent('folderlist', { path: parentPath, files: result.objs });
      } else {
        cb(err);
      }
    });
    ...
    if (this.isTempFileName(pattern)) {
      ...
      this.exists(pattern, function (err, exists) {
        if (err) {
          cb(err);
        } else if (exists) {
          self.createFileInstance(pattern, function (err, file) {
            cb(err, [file]);
          });
        } else {
          // path doesn't exist, don't try to create a file instance for it
          cb(err, []);
        }
      });
    }
 ...
 */
JCRTree.prototype._createFile = function (name, cb) {
  var self = this;
  var logger = this.getLogger();
  ...
  this.share.createFileResource(this, name, null, function (err) {
    if (err) {
      cb(err);
      return;
    }
 ...
 */
JCRTree.prototype._createDirectory = function (name, cb) {
  var self = this;
  var logger = this.getLogger();
  ...
  this.share.createDirectoryResource(this, name, function (err) {
    if (err) {
      cb(err);
      return;
    }
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
JCRTree.prototype._delete = function (name, cb) {
  var logger = this.getLogger();
  ...
  this.share.deleteResource(this, name, true, cb);
};
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
JCRTree.prototype._deleteDirectory = function (name, cb) {
  var logger = this.getLogger();
  var self = this;
  ...
  this.share.deleteResource(this, name, false, function (err) {
    if (err) {
      cb(err);
      return;
    }
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
JCRTree.prototype._rename = function (oldName, newName, cb) {
  var logger = this.getLogger();
  ...
  this.share.renameResource(this, oldName, newName, cb);
};
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
JCRTree.prototype._refresh = function (folderPath, deep, cb) {
  var logger = this.getLogger();
  logger.debug('[%s] tree.refresh %s, %d', this.share.config.backend, folderPath, deep);

  // invalidate cache
  this.share.invalidateContentCache(this, utils.getParentPath(folderPath), deep);
