...
var RQFile = function (fileConnection, openFile, tree) {
  if (!(this instanceof RQFile)) {
    return new RQFile(fileConnection, openFile, tree);
  }
...
RQFile.prototype.cacheFile = function (cb) {
  ...
  function setOpenFile(newOpenFile, callback) {
    if (newOpenFile) {
      self.openFile.close(function (err) {
        if (err) {
          logger.warn('error while attempting to close current open file', err);
        }
        self.openFile = newOpenFile;
        fileConnection.setSourceFileConnection(newOpenFile.getFileConnection());
        callback();
      });
    } else {
      callback();
  ...
  if (!fileConnection.isSyncDone()) {
    share.cacheFile(self, function (err, cachedFile) {
      if (err) {
        ...
        setOpenFile(cachedFile, function () {
