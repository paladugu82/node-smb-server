 ...
 */
var RQLocalFile = function (fileConnection, source, tree) {
  if (!(this instanceof RQLocalFile)) {
    return new RQLocalFile(fileConnection, source, tree);
  }

  File.call(this, fileConnection, tree);
  this.source = source;
  this.data = fileConnection.getCacheData();
...
/**
 * Returns a value indicating whether the local file is in conflict.
 * @return {Boolean} TRUE if the file is in conflict, otherwise false;
 */
RQLocalFile.prototype.isConflict = function () {
  return this.data.conflict ? true : false;
};
...
RQLocalFile.prototype.canDelete = function (cb) {
  ...
  if (self.isDirectory()) {
    ...
    logger.debug('%s is a file, checking lastModified(%d)==lastModifiedAtCache(%d), not isCreatedLocally(%s), has a downloadedRemoteModifiedDate(%d), and not conflict(%s)', path, self.lastModified(), lastModifiedAtCache, self.isCreatedLocally(), self.getDownloadedRemoteModifiedDate(), self.isConflict());

    var hasRemoteModified = self.getDownloadedRemoteModifiedDate() ? true : false;
    cb(null, self.source.lastModified() == lastModifiedAtCache && !self.isCreatedLocally() && hasRemoteModified && !self.isConflict());
  }
};
