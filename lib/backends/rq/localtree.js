...
var RQLocalTree = function (share, localTree, rqTree) {
  ...
  Tree.call(this, rqTree.treeConnection, rqTree.context);
};
...
function _writeWorkData(workFile, cacheInfo, cb) {
  ...
  logger.info('creating cache info file %s', filePath);
...
/**
 * Deletes the work file for the corresponding file, if it exists.
 * @param {string} name Path of the file whose work file should be deleted.
 * @param {function} delCb Will be invoked when the operation is complete.
 * @private
 */
function _deleteWorkFile(name, delCb) {
  var self = this;
  var filePath = self.getInfoFilePath(name);
...
/**
 * Retrieves the tree that the local tree is using as its source.
 * @returns {Tree} A Tree instance.
 */
RQLocalTree.prototype.getSourceTree = function () {
  return this.source;
};
 ...
 */
RQLocalTree.prototype.download = function (remote, name, options, cb) {
  ...
  function refreshWorkFile(refreshCb) {
    if (options.refresh) {
      _deleteWorkFile.call(self, name, refreshCb);
    } else {
      refreshCb();
    }
  }
  ...
  remote.open(name, function (err, remoteFile) {
    if (err) {
      ...
      self.share.fetchResource(self.getContext(), name, function (err, localPath) {
        if (err) {
...
/**
 * Sets a flag in the file's cache info that indicates it is in conflict.
 * @param {String} path Full path of a file.
 * @param {Boolean} isConflict The value to set as the file's conflict flag.
 * @param {Function} cb Invoked when the operation is complete.
 * @param {Error} cb.err Truthy if there was an error during the operation.
 */
RQLocalTree.prototype.setFileConflict = function (path, isConflict, cb) {
  var logger = this.getLogger();
  logger.debug('setFileConflict: entering with path=%s, isConflict=%s', path, isConflict);
  var self = this;
  var infoFile = self.getInfoFilePath(path);
  self.source.open(infoFile, function (err, localFile) {
    if (err) {
      ...
      RQLocalFile.readCacheInfo(localFile, function (err, cacheInfo) {
        if (err) {
          ...
          cacheInfo.conflict = isConflict;
          _writeWorkData.call(self, localFile, cacheInfo, cb);
        }
      });
    }
  });
...
RQLocalTree.prototype.refreshCacheInfo = function(name, remoteFile, cb) {
  ...
  _deleteWorkFile.call(self, name, function () {
          ...
          localFile.close(function (closeErr) {
            ...
            if (err) {
              ...
              workFile.close(function (err) {
                if (err) {
                  logger.error('unable to close work file %s', name, err);
                }
                cb();
              });
            }
...
RQLocalTree.prototype.createFromSource = function (source, remote, isCreated, cb) {
  ...
  _createWorkFile.call(self, source, isCreated, false, remote, function (err, workFile) {
    if (err) {
      ...
      RQLocalFileConnection.createFileInstance(source, workFile, self, cb);
    }
  });
};
 ...
 */
RQLocalTree.prototype._open = function (name, cb) {
  var logger = this.getLogger();
  var self = this;
  if (self.isDownloading(name)) {
    ...
    self.source._open(name, function (err, file) {
      if (err) {
        cb(err);
      } else {
 ...
 */
RQLocalTree.prototype._list = function (pattern, cb) {
  var self = this;
  self.source._list(pattern, function (err, files) {
    if (err) {
      cb(err);
    } else {
      var result = [];
 ...
 */
RQLocalTree.prototype._createFile = function (name, cb) {
  var logger = this.getLogger();
  var self = this;
  ...
  if (self.isDownloading(name)) {
    ...
    if (self.cacheInfoOnly) {
      self.source._open(name, _handleFile);
    } else {
      self.source._createFile(name, _handleFile);
    }
  }
};
 ...
 */
RQLocalTree.prototype._createDirectory = function (name, cb) {
  var self = this;
  this.source._createDirectory(name, function (err, dir) {
    if (err) {
      cb(err);
    } else {
      self.createFromSource(dir, null, false, cb);
    }
  });
};
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
RQLocalTree.prototype._delete = function (name, cb) {
  this.deleteWithForce(name, false, cb);
};
 ...
 *
 * @param {String} name file name
 * @param {boolean{ force If true, deletes the file regardless of the cacheInfoOnly setting.
 * @param {Function} cb callback called on completion
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
RQLocalTree.prototype.deleteWithForce = function (name, force, cb) {
  var logger = this.getLogger();
  var self = this;

  function doDelete(deleteCb) {
    if (self.cacheInfoOnly && !force) {
      // don't attempt to delete source
      deleteCb();
    } else {
      self.source._delete(name, function (err) {
        if (err) {
          cb(err);
        } else {
          deleteCb();
    ...
    doDelete(function () {
      self.cacheInfoExists(name, function (err, exists) {
        if (err) {
          ...
          self.source._delete(self.getInfoFilePath(name), cb);
        } else {
          cb();
        }
      });
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
RQLocalTree.prototype._deleteDirectory = function (name, cb) {
  this.deleteDirectoryWithForce(name, false, cb);
};
 ...
 *
 * @param {String} name directory name
 * @param {Boolean} force if true, deletes the directory regardless of the cacheInfoOnly setting.
 * @param {Function} cb callback called on completion
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
RQLocalTree.prototype.deleteDirectoryWithForce = function (name, force, cb) {
  var logger = this.getLogger();
  var self = this;

  function _deleteWorkDir(delCb) {
    var workDir = Path.join(name, consts.WORK_DIR);
    self.source._exists(workDir, function (err, exists) {
      if (err) {
        cb(err);
      } else if (exists) {
        logger.debug('%s processing work directory for removal', workDir);
        // delete any dangling work files
        self.source._list(Path.join(workDir, '*'), function (err, workFiles) {
          if (err) {
            cb(err);
          } else {
            async.eachSeries(workFiles, function (workFile, eachCb) {
              logger.debug('%s removing dangling workfile', workFile.getPath());
              self.source._delete(workFile.getPath(), eachCb);
            }, function (err) {
              if (err) {
                cb(err);
              } else {
                logger.debug('%s deleting work directory', workDir);
                self.source._deleteDirectory(workDir, function (err) {
                  if (err) {
                    cb(err);
                  } else {
                    delCb();
  ...
  self.list(Path.join(name, '*'), function (err, items) {
    if (err) {
      ...
      self.source._deleteDirectory(name, cb);
    } else {
...
RQLocalTree.prototype.renameExt = function (oldName, newName, newRemote, cb) {
  ...
  function _deleteCacheInfo(deleteName, delCb) {
    self.cacheInfoExists(deleteName, function (err, exists) {
      if (err) {
        ...
        self.source._delete(self.getInfoFilePath(deleteName), function (err) {
          if (err) {
            cb(err);
          } else {
            delCb();
  ...
  function _doRename(renameCb) {
    ...
    if (self.cacheInfoOnly) {
      ...
      self.source._rename(oldName, newName, function (err) {
        if (err) {
          cb(err);
        } else {
          renameCb();
    ...
    _doRename(function () {
      _deleteCacheInfo(oldName, function () {
        _deleteCacheInfo(newName, function () {
          ...
          self.source._open(newName, function (err, renamed) {
            if (err) {
              cb(err);
            } else {
              // if the target work file already existed, don't flag the file as newly created.
              _createWorkFile.call(self, renamed, newCreated, !newCreated, newRemote, function (createErr, workFile) {
                renamed.close(function (closeErr) {
                  ...
                  if (createErr) {
                    ...
                    if (workFile) {
                      workFile.close(function (err) {
                        if (err) {
                          logger.warn('unexpected error while trying to close work file', err);
                        }
                        cb();
                      });
                    } else {
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
RQLocalTree.prototype._rename = function (oldName, newName, cb) {
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
RQLocalTree.prototype._refresh = function (folderPath, deep, cb) {
  this.source._refresh(folderPath, deep, cb);
};
