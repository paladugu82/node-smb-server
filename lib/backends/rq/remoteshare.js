...
RQRemoteShare.prototype.fetchResource = function (path, cb) {
  var self = this;
  DAMShare.prototype.fetchResource.call(this, path, function (err, tmpPath) {
    if (err) {
      cb(err);
    } else {
      ...
      logger.warn('successfully downloaded %s to temp location %s. moving to permanent cache at %s', path, tmpPath, localPath);
