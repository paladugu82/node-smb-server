 ...
 */
RequestQueue.prototype.getRequests = function (tree, path, callback) {
  var log = tree.getLogger();
 ...
 */
RequestQueue.prototype.getAllRequests = function (tree, callback) {
  var rqlog = tree.getRqLogger();
 ...
 */
RequestQueue.prototype.exists = function (tree, path, name, callback) {
  var rqlog = tree.getRqLogger();
 ...
 */
RequestQueue.prototype.incrementRetryCount = function (tree, path, name, delay, callback) {
  var rqlog = tree.getRqLogger();
  var log = tree.getLogger();
...
function _emitEvent(tree, eventName, path) {
  this.emit(eventName, { path: path, tree: tree });
}
 ...
 */
RequestQueue.prototype.removeRequest = function (tree, path, name, callback) {
  var log = tree.getLogger();
  var rqlog = tree.getRqLogger();
  ...
  this.db.remove({path: path, name: name}, {}, function (err, numRemoved) {
    if (err) {
      ...
      _emitEvent.call(self, tree, 'itemupdated', Path.join(path, name));
 ...
 * @param {String|Error} callback.err If non-null, indicates that an error occurred.
 */
RequestQueue.prototype.completeRequest = function (tree, path, name, callback) {
  var log = tree.getLogger();
  var rqlog = tree.getRqLogger();
 ...
 */
RequestQueue.prototype.getProcessRequest = function (tree, expiration, maxRetries, callback) {
  var log = tree.getLogger();
  var rqlog = tree.getRqLogger();
 ...
 */
RequestQueue.prototype.updatePath = function (tree, oldPath, newPath, callback) {
  var log = tree.getLogger();
  var rqlog = tree.getRqLogger();
    ...
    if (err) {
      ...
      var updateDoc = function (index) {
        if (index < docs.length) {
          ...
          _emitEvent.call(self, tree, 'pathupdated', oldPath);
          if (docs.length) {
 ...
 * @param {String|Error} callback.err Will be truthy if there was an error while updating.
 */
RequestQueue.prototype.removePath = function (tree, path, callback) {
  var log = tree.getLogger();
  var rqlog = tree.getRqLogger();
  ...
  this.db.remove(self.getFindPathFilter(path), {multi: true}, function (err, numAffected) {
    if (err) {
      ...
      _emitEvent.call(self, tree, 'pathupdated', path);
      if (numAffected) {
 ...
 * @param {String|Error} callback.err Will be truthy if there was an error while updating.
 */
RequestQueue.prototype.copyPath = function (tree, oldPath, newPath, callback) {
  var log = tree.getLogger();
  var rqlog = tree.getRqLogger();
 ...
 */
RequestQueue.prototype.queueRequest = function (tree, options, callback) {
  var log = tree.getLogger();
  var rqlog = tree.getRqLogger();
  ...
  if (fullPath.match(/\/\./g)) {
    ...
    var handleResult = function (err, sendUpdate, resultPath, resultName) {
      if (err) {
        ...
        if (sendUpdate) {
          _emitEvent.call(self, tree, 'itemupdated', Path.join(resultPath, resultName));
        }
