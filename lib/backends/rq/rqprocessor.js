...
function _processItemMethod (tree, parentPath, name, remotePrefix, localPrefix, method, chunksInMb, processCb) {
  ...
  if (url.match(/\/\./g)) {
    ...
    /* Define these variables here. Values will be set later*/
	  ...
	  var getReqCallback = function (err, resp) {
			if (err) {
				// failed
					...
					fs.stat(localPath, function (err, stats) {
						if (err) {
							...
							var checkAsync = function() {	
							 ...
							 /* File uploaded completely. Complete the request now*/
									completeRequest();
									return false;
								}
								return true;
							...
							async.whilst(checkAsync, function (asyncCb)  {
								
								var req = getRequest( function(err, resp) {		
									if(getReqCallback(err,resp)) {
										asyncCb();
									}
