...
function RQProcessor(tree, options) {
  ...
  this.rq.on('itemupdated', function (data) {
    ...
    var itemTree = data.tree;
    var logger = itemTree.getLogger();
...
RQProcessor.prototype.sync = function (config, cb) {
  ...
  self.currCb = cb;
  var tree = self.tree.createTree(new SMBContext().withLabel('RQProcessor.sync'));
  ...
  var processItemMethod = function (item, method, processCb) {
    ...
    var handleError = function (path, method, err, immediateFail) {
      ...
      removeActiveUpload();
      if (immediateFail) {
        self.rq.completeRequest(tree, item.path, item.name, function (err) {
          if (err) {
        ...
        self.rq.incrementRetryCount(tree, item.path, item.name, config.retryDelay, function (err) {
          if (err) {
  ...
  self.rq.getProcessRequest(tree, config.expiration, config.maxRetries, function (err, item) {
    if (err) {
      cb(err);
    } else {
      if (item) {
