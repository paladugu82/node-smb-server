...
RQProcessor.prototype.sync = function (config, cb) {
  ...
  var processItemMethod = function (item, method, processCb) {
    ...
    var handleError = function (path, method, err, immediateFail) {
      ...
      _emitContextEvent.call(self, 'syncerr', {path: refPath, file: path, method: method, err: err}, context);
      removeActiveUpload();
      if (immediateFail) {
        self.rq.completeRequest(context, item.path, item.name, function (err) {
          if (err) {
        ...
        self.rq.incrementRetryCount(context, item.path, item.name, config.retryDelay, function (err) {
          if (err) {
    ...
    var endSync = function () {
      logger.info('finished sync of file %s', refPath);
      _emitContextEvent.call(self, 'syncend', {path: refPath, file: path, method: method}, context);
      self.sync(config, processCb);
    };
    var completeRequest = function () {
      self.rq.completeRequest(context, item.path, item.name, function (err) {
        if (err) {
          ...
          removeActiveUpload();
          self.tree.share.invalidateContentCache(tree, self.unicodeNormalize(item.path), true);
          if (method != 'DELETE') {
            tree.refreshWorkFiles(self.unicodeNormalize(refPath), function (err) {
              if (err) {
    ...
    logger.info('beginning sync of file %s', refPath);
    _emitContextEvent.call(self, 'syncstart', {path: refPath, file: path, method: method}, context);
    ...
    if (url.match(/\/\./g)) {
      ...
      _checkExists.call(self, tree, method, refPath, function (err, exists) {
        if (err) {
          handleError(path, method, err);
        } else {
