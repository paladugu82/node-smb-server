...
function _processItemMethod (tree, parentPath, name, remotePrefix, localPrefix, method, chunksInMb, processCb) {
  var self = this;
  ...
  if(method == 'POST' || method == 'PUT') {
    var remoteUrl = this.share.getRemoteContentUrl();
    if (remoteUrl.charAt(remoteUrl.length - 1) == '/') {
      remoteUrl = remoteUrl.substr(0, remoteUrl.length - 1);
    }
    if (parentPath != '/') {
      remoteUrl += encodeURI(utils.normalizeSMBFileName(parentPath));
    }
    remoteUrl += '.createasset.html';
    logger.debug('CreateAssetUrl for the file %s is : %s', serverPath, remoteUrl);
  }
  ...
  var completeRequest = function (err) {
    if (err) {
      handleError(localPath, method, err, immediateFail);
      return;
    }
    removeActiveUpload();
  ...
  if (url.match(/\/\./g)) {
    ...
    options.headers[JCR.ACTION_HEADER] = (method == 'POST' ? JCR.ACTION_CREATEFILE : (method == 'PUT' ? JCR.ACTION_UPDATEFILE : JCR.ACTION_DELETEFILE));

    /* Define these variables here. Values will be set later*/
		var uploadNextChunk;
		var read, stats;

    var getReqCallback = function (err, resp, cb) {
      if (resp.statusCode == 423) {
        logger.debug('path [%s] name [%s] received locked status, indicating file is checked out', serverPath, name);
        err = 'Asset is checked out by another user';
        immediateFail = true;
      } else if (resp.statusCode != 200 && resp.statusCode != 201) {
        logger.debug('received response with invalid status code %d', resp.statusCode);
        err = 'unexpected status code: ' + resp.statusCode;
      } else if (!err) {
        logger.debug('path [%s] name [%s] chunk completed', serverPath, name);
      }
      cb(err);
    };

		var getRequest = function (cb, totalSize) {
			/* For updating the files, change the URL and method to createassethtml and content type to multipart formdata*/
			if(method == 'POST' || method == 'PUT') {
				options.url = remoteUrl;
				options.headers['content-type'] = 'multipart/form-data';
				options.headers['x-chunked-content-type'] = utils.lookupMimeType(serverPath);
				options.headers['x-chunked-total-size'] = totalSize;
			}
			return webutils.submitRequest(options, cb);
		};
				...
				if (method == 'POST' || method == 'PUT') {
					...
					fs.stat(localPath, function (err, stats) {
						if (err) {
							...
							var chunkSizeFixed = 1024*1024*chunksInMb;

							async.whilst(function () { return chunkOffset < fileSize; }, function (asyncCb)  {
								
								var req = getRequest( function(err, resp) {
								  getReqCallback(err, resp, asyncCb);
								}, fileSize);
								var form = req.form();
								if(method == 'PUT') {
									logger.debug('Updating the file %s, Use post for CreateAssetServelet',serverPath);
									req.method = 'POST';
									form.append('replaceAsset','true');
								}
								var chunkSize = chunkOffset+ chunkSizeFixed < fileSize ? chunkSizeFixed : fileSize - chunkOffset;
								logger.debug('Uploading chunks to the file % of size %s Chunk curent offset %s chunksize %s',serverPath, fileSize, chunkOffset, chunkSize);
								read = fs.createReadStream(localPath,{'start':chunkOffset, 'end':chunkSize+chunkOffset-1});
								read.on('error', function (err) {
									handleError(localPath, method, err);
								}); 
							/* if filesize is less than the chunk, upload file at once*/
								if(chunkSizeFixed < fileSize) {
									form.append('file@Offset', chunkOffset);
								}
								form.append('chunk@Length', chunkSize);
								form.append('file@Length', fileSize);
								if(chunkOffset >= fileSize) {
									form.append('file@Completed', 'true');
								}
								form.append('file', read);
								webutils.monitorTransferProgress(read, serverPath, localPath, fileSize, function (progress) {
									/* Add the chunk offset, thats what is already uploaded*/
									progress.read = progress.read + chunkOffset - chunkSize;
									logger.debug('%s: read %d of %d bytes, upload %d percent complete, rate of %d bytes/sec', serverPath, progress.read, fileSize, Math.round(progress.read / fileSize * 100), progress.rate);
					...
					getRequest(function (err, resp) {
					  getReqCallback(err, resp, completeRequest);
          });
				}
			}
		});
...
function _syncItem(config, tree, item, parentPath, name, method, remotePrefix, localPrefix, cb) {
  ...
  var chunkSizeinMb = config.chunkUploadSize ? config.chunkUploadSize : 1;
  _processItemMethod.call(self, tree, parentPath, name, remotePrefix, localPrefix, method, chunkSizeinMb, function (err) {
    if (item) {
      if (err) {
        var immediateFail = false;
        if (err.err) {
