 ...
 */
RQTree.prototype._exists = function (name, cb) {
          ...
          if (err) {
            ...
            self.isQueuedFor(name, ['DELETE'], function (err, deleted) {
              if (err) {
                ...
                self.remote.exists(name, function (err, remoteResult) {
                  if (err) {
 ...
 */
RQTree.prototype._open = function (name, cb) {
  var logger = this.getLogger();
  var rqlog = this.getRqLogger();
...
function _prepListData(tree, pattern, cb) {
  ...
  tree.rq.getRequests(tree.getContext(), data.parentPath, function (err, requests) {
    if (err) {
      cb(err);
    } else {
    ...
    if (err) {
      ...
      async.eachSeries(remoteFiles, function (remoteFile, eachCb) {
        if (tree.isTempFileName(remoteFile.getPath())) {
          ...
          if (existingRequests[remoteFile.getName()] != 'DELETE') {
 ...
 */
RQTree.prototype._list = function (pattern, cb) {
  var logger = this.getLogger();
  var rqlog = this.getRqLogger();
...
RQTree.prototype.queueDataExists = function (name, cb) {
  this.rq.exists(this.getContext(), utils.getParentPath(name), utils.getPathName(name), cb);
};
 ...
 */
RQTree.prototype._createFile = function (name, cb) {
  var logger = this.getLogger();
  var rqlog = this.getRqLogger();
 ...
 */
RQTree.prototype._createDirectory = function (name, cb) {
  var logger = this.getLogger();
  var rqlog = this.getRqLogger();
  ...
  self.local.createDirectory(name, function (err, file) {
    if (err) {
      ...
      if (!self.isTempFileName(name)) {
        ...
        self.remote.createDirectory(name, function (err, remoteDir) {
          if (err) {
            self.handleErr(cb, err);
          } else {
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
RQTree.prototype._delete = function (name, cb) {
  var logger = this.getLogger();
  var rqlog = this.getRqLogger();
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
RQTree.prototype._deleteDirectory = function (name, cb) {
  var logger = this.getLogger();
  var rqlog = this.getRqLogger();
  ...
  var deleteRemote = function (callback) {
    if (!self.isTempFileName(name)) {
      ...
      self.remote.deleteDirectory(name, callback);
    } else {
      callback();
    }
  };
          ...
          if (err) {
            ...
            deleteRemote(function (err) {
              if (err) {
                ...
                self.rq.removePath(self.getContext(), name, function (err) {
                  if (err) {
                    self.handleErr(cb, err);
                  } else {
                    cb();
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
RQTree.prototype._rename = function (oldName, newName, cb) {
  var logger = this.getLogger();
  var rqlog = this.getRqLogger();
  ...
  var renameRemoteDir = function (callback) {
    ...
    if (!self.isTempFileName(oldName) && !self.isTempFileName(newName)) {
      ...
      self.remote.rename(oldName, newName, callback);
    } else {
            ...
            if (info.isDir) {
              ...
              renameRemoteDir(function (err) {
                if (err) {
                  ...
                  self.rq.updatePath(self.getContext(), oldName, newName, function (err) {
                    if (err) {
                      self.handleErr(cb, err);
                    } else {
 ...
 * over the share's maximum threshold. And also checks for any initial conflicts and sends a syncconflict event
 ...
 */
RQTree.prototype.checkCacheSizeAndConflicts = function (maxCacheSize, forceEvent, findConflicts) {
  var logger = this.getLogger();
  var rqlog = this.getRqLogger();
  logger.debug('checkCacheSize entering with maxCacheSize: %s, forceEvent: %s findConflicts: %s', maxCacheSize, forceEvent, findConflicts);
  ...
  self.traverseDirectory('/', function (parent, directory, dirCb) {
    ...
    if(findConflicts) {
      // checking if the file is a conflict too
      file.canDelete(function (err, canDelete) {
        if(!err && !canDelete) {
          logger.debug('A conflict has been found for file %s', file.getPath());
          self.emitSyncConflict(file.getPath());
        }
      });
    }
 ...
 */
RQTree.prototype.setFileConflict = function (path, isConflict, cb) {
  this.local.setFileConflict(path, isConflict, cb);
};
