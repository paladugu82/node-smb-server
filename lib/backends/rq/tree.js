...
RQTree.prototype.isQueuedFor = function (name, methods, cb) {
  var self = this;
  self.rq.getRequests(this, utils.getParentPath(name), function (err, lookup) {
    if (err) {
  ...
  this.local.exists(name, function (err, result) {
    if (err) {
      ...
      if (result) {
        ...
        self.rq.getRequests(self, utils.getParentPath('/'), function (err, lookup) {
          if (err) {
            self.handleErr(cb, err);
          } else {
...
function _prepListData(tree, pattern, cb) {
  ...
  tree.rq.getRequests(tree, tree.remoteEncodePath(data.parentPath), function (err, requests) {
    if (err) {
      cb(err);
    } else {
...
RQTree.prototype.queueDataExists = function (name, cb) {
  ...
  self.rq.exists(self, utils.getParentPath(encoded), utils.getPathName(encoded), cb);
};
...
RQTree.prototype.queueData = function (name, method, newName, moreOpts, cb) {
  ...
  if (!isTempFile) {
    this.rq.queueRequest(self, options, function (err) {
      if (err) {
          ...
          if (err) {
            ...
            deleteRemote(function (err) {
              if (err) {
                ...
                self.rq.removePath(self, self.remoteEncodePath(name), function (err) {
                  if (err) {
                    self.handleErr(cb, err);
                  } else {
                    cb();
            ...
            if (info.isDir) {
              ...
              renameRemoteDir(function (err) {
                if (err) {
                  ...
                  self.rq.updatePath(self, self.remoteEncodePath(oldName), self.remoteEncodePath(newName), function (err) {
                    if (err) {
                      self.handleErr(cb, err);
                    } else {
