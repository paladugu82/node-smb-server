...
function _processDirectory(tree, toProcess, force, cb) {
  var logger = tree.getLogger();
  var processDirs = [{
    name: toProcess
  }];
  ...
  async.whilst(function () {
      ...
      if (err) {
        ...
        async.eachSeries(items, function (item, eachCb) {
          if (item.isDirectory()) {
            ...
            _deleteFile(tree, item, force, function (err, wasDeleted) {
              if (err) {
                eachCb(err);
              } else {
                if (!wasDeleted) {
    ...
    if (err) {
      ...
      _deleteDirs(tree, processDirs, skipDelete, force, cb);
    }
  });
}

function _deleteDirs(tree, processDirs, skipDelete, force, cb) {
  var logger = tree.getLogger();
  ...
  async.eachSeries(processDirs, function (dir, eachCb) {
    ...
    if (skipDelete[dirName]) {
      ...
      if (dirName != '/') {
        ...
        tree.local.deleteDirectoryWithForce(dirName, force, eachCb);
      } else {
...
RQTree.prototype.deleteLocalDirectoryRecursiveExt = function (name, force, cb) {
  ...
  _processDirectory(self, name, force, function (err) {
    if (err) {
      self.handleErr(cb, err);
    } else {
      cb();
