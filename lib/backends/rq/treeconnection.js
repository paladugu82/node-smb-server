...
var RQTreeConnection = function (share, remote, options) {
  ...
  this.share.on('uploadasset', function (eventData) {
  ...
  this.share.on('downloadasset', function (eventData) {
    _downloadAsset.call(self, eventData);
  });
  ...
  this.share.on('checkcachesize', function (eventData) {
    ...
    tree.checkCacheSize(eventData.maxCacheSize, eventData.forceEvent);
  });

  this.share.on('cancelupload', function (eventData) {
    self.processor.abortUpload(eventData.path);
  });
  ...
  function handleExists(callback) {
    tree.local.exists(path, function (err, exists) {
      ...
      if (exists) {
        ...
        } else if (data.openIfExists) {
          self.share.emitShareEvent('openasset', {path: path});
        }
      } else {
        callback();
      }
