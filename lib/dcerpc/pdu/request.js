...
var put = require('put');
var logger = require('../../logging').getLogger('dcerpc');
...
function handle(hdr, buf, pipe, server, cb) {
  ...
  var syntaxSpec = pipe.getSyntaxSpec();
  var iface = consts.SUPPORTED_INTERFACES[syntaxSpec.uuid][syntaxSpec.version + '.' + syntaxSpec.versionMinor];
  var funcName = iface[hdr.opnum];
  if (!funcName) {
    logger.error('encountered unsupported operation 0x%s (%s v%d.%d)', hdr.opnum.toString(16), consts.SUPPORTED_INTERFACES[syntaxSpec.uuid].name, syntaxSpec.version, syntaxSpec.versionMinor);
    onFault(hdr, consts.NCA_UNSPEC_REJECT, cb);
    return;
  }

  logger.debug('%s (%s v%d.%d)', funcName, consts.SUPPORTED_INTERFACES[syntaxSpec.uuid].name, syntaxSpec.version, syntaxSpec.versionMinor);
  ...
  switch (funcName) {
    case 'NetShareEnumAll':
      ...
      break;
    default:
      logger.error('encountered unsupported operation %s (%s v%d.%d)', funcName, consts.SUPPORTED_INTERFACES[syntaxSpec.uuid].name, syntaxSpec.version, syntaxSpec.versionMinor);
      onFault(hdr, consts.NCA_UNSPEC_REJECT, cb);
  }
}
