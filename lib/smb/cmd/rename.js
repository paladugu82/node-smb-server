...
var put = require('put');
var logger = require('../../logging').getLogger('smb');
...
function handle(msg, commandId, commandParams, commandData, commandParamsOffset, commandDataOffset, connection, server, cb) {
  ...
  var context = server.createContext(msg.header.tid);
  cb = new CommandContext(context).withMoveInfo(connection, msg.oldFileName, msg.newFileName, msg.header.tid).wrapHandlerCallback(msg, commandId, cb);
  ...
  var result;
  var tree = server.getTree(msg.header.tid, context);
  if (!tree) {
    result = {
