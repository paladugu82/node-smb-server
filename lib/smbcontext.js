...
function _getLog(name) {
  if (!this.logs[name]) {
    this.logs[name] = new WrappedLog(loggers.getLogger(name), name, this.getLabel(), this.requestId);
  }
...
SMBContext.prototype.getDetails = function () {
  return this.details;
};
 ...
 * @param {String} label The arbitrary label assigned to the context.
 * @returns {SMBContext} The context.
 */
...
/**
 * Configures the context to use a given detail item.
 * @param {String} name The name of the detail item.
 * @param {Object} value The value to associated with the detail item.
 * @returns {SMBContext} The context.
 */
SMBContext.prototype.withDetail = function (name, value) {
