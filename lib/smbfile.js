 ...
 * @param {SMBFileConnection} fileConnection
 * @constructor
 */
function SMBFile(spiFile, smbTree, fileConnection) {
  this.spiFile = spiFile;
  this.smbTree = smbTree;
  this.createAction = fileConnection.getCreateAction();
  this.attributes = 0;
  ...
  this.fid = fileConnection.getFid();
  this.fileConnection = fileConnection;
  this.deleteOnClose = false;
}

SMBFile.prototype.getLogger = function () {
  return this.spiFile.getLogger();
};
...
SMBFile.prototype.getSyntaxSpec = function () {
  return this.fileConnection.getSyntaxSpec();
};

SMBFile.prototype.setSyntaxSpec = function (syntaxSpec) {
  this.fileConnection.setSyntaxSpec(syntaxSpec);
};
...
SMBFile.prototype.read = function (buffer, offset, length, position, cb) {
  this.getLogger().debug('[fid=%d] file.read %s offset=%d, length=%d, position=%d', this.fid, this.getPath(), offset, length, position);
...
SMBFile.prototype.write = function (data, position, cb) {
  this.getLogger().debug('[fid=%d] file.write %s data.length=%d, position=%d', this.fid, this.getPath(), data.length, position);
...
SMBFile.prototype.delete = function (cb) {
  this.getLogger().debug('[fid=%d] file.delete %s', this.fid, this.getPath());
...
SMBFile.prototype.setLength = function (length, cb) {
  this.getLogger().debug('[fid=%d] file.setLength %s length=%d', this.fid, this.getPath(), length);
...
SMBFile.prototype.setDeleteOnClose = function () {
  this.getLogger().debug('[fid=%d] file.setDeleteOnClose %s', this.fid, this.getPath());
...
SMBFile.prototype.flush = function (cb) {
  this.getLogger().debug('[fid=%d] file.flush %s', this.fid, this.getPath());
...
SMBFile.prototype.close = function (cb) {
  this.getLogger().debug('[fid=%d] file.close %s deleteOnClose=%d', this.fid, this.getPath(), this.deleteOnClose);
  var self = this;
