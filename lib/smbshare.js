...
var SMBTreeConnection = require('./smbtreeconnection');
...
SMBShare.prototype.onServerEvent = function (context, eventName, data, callback) {
  this.spiShare.onServerEvent(context, eventName, data, callback);
};
...
SMBShare.prototype.connect = function (session, shareLevelPassword, cb) {
  ...
  this.spiShare.connect(session, shareLevelPassword, function (err, tree) {
    if (err) {
      ...
      cb(null, new SMBTreeConnection(self.smbServer, self, tree));
    }
  });
};
