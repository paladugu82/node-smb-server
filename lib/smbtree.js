...
SMBTree.prototype.getContext = function () {
  return this.context;
};
...
function _createSmbFileInstance(file, createAction, callback) {
  var result;
  try {
    result = this.treeConnection.createFileInstance(this, file, createAction);
  } catch (e) {
    callback(e);
    return;
  }
...
SMBTree.prototype.openOrCreate = function (name, createDisposition, openTargetDirectory, cb) {
  ...
  function open(callback) {
    self.spiTree.open(name, function (err, file) {
      ...
      _createSmbFileInstance.call(self, file, openAction, function (err, result) {
        if (err) {
          callback(err);
        } else if (openAction === common.FILE_OVERWRITTEN) {
...
SMBTree.prototype.open = function (name, cb) {
  ...
  this.spiTree.open(utils.normalizeSMBFileName(name), function (err, file) {
    if (err) {
      ...
      _createSmbFileInstance.call(self, file, common.FILE_OPENED, cb);
    }
  });
};
  ...
  this.spiTree.list(npattern, function (err, files) {
    if (err) {
      ...
      var results = files.map(function (file) {
        return new SMBFile(file, self, new SMBFileConnection(self.smbServer, self.smbShare, self.treeConnection, file.getFileConnection()));
      });
      cb(null, results);
...
SMBTree.prototype.createFile = function (name, cb) {
  ...
  this.spiTree.createFile(nname, function (err, file) {
    ...
    _createSmbFileInstance.call(self, file, common.FILE_CREATED, cb);
