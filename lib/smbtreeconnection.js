...
SMBTreeConnection.fidCounter = common.MIN_FID;

SMBTreeConnection.prototype.createContext = function () {
  return this.spiTreeConnection.createContext();
};
...
function _getNextFid(smbTree) {
  var logger = smbTree.getContext().default();
  var fid = SMBTreeConnection.fidCounter++;
  // initially wrap around the fid if needed
  if (fid > common.MAX_FID) {
    logger.debug('ran out of fids. wrapping around to minimum of range');
    fid = common.MIN_FID;
  }

  // make sure the fid isn't already in use by another file
  var startId = fid;
  var first = true;
  while (this.files[fid]) {
    if (!first && fid == startId) {
      // we've looped through the entire range of available fid's. no more id's are available
      logger.error('unable to generate a file id: no more available');
      throw new Error('unable to generate a file id: no more available');
    }
    first = false;
    fid++;
    ...
    if (fid > common.MAX_FID) {
      logger.debug('ran out of fids while searching for the next available id. wrapping around to minimum of range');
      fid = common.MIN_FID;
    }
  }
  if (fid != startId) {
    logger.debug('initial fid %s was in use. found the next available and assigned fid %s to file', startId, fid);
    if (fid > startId) {
      var traversed = fid - startId;
      if (traversed > 10) {
        logger.warn('server had to traverse through %d fids to find unused', traversed);
      }
    } else {
      var traversed = common.MAX_FID - startId;
      traversed += fid;
      if (traversed > 10) {
        logger.warn('server had to traverse through %d fids to find ununsed', traversed);
      }
    }
  }
  return fid;
...
SMBTreeConnection.prototype.createFileInstance = function (smbTree, spiFile, openAction) {
  var fid = _getNextFid.call(this, smbTree);
