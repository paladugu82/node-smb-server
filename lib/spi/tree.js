...
Tree.prototype.exists = function (name, cb) {
  this._exists(this.unicodeNormalize(name), cb);
};

/**
 * Private implementation of exists method. Should be overridden by child classes.
 *
 * @param {String} name file name
 * @param {Function} cb callback called with the result
 * @param {SMBError} cb.error error (non-null if an error occurred)
 * @param {Boolean} cb.exists true if the file exists; false otherwise
 */
Tree.prototype._exists = function (name, cb) {
...
Tree.prototype.open = function (name, cb) {
  this._open(this.unicodeNormalize(name), cb);
};

/**
 * Private implementation of open method. Should be overridden by child classes.
 *
 * @param {String} name file name
 * @param {Function} cb callback called with the opened file
 * @param {SMBError} cb.error error (non-null if an error occurred)
 * @param {File} cb.file opened file
 */
Tree.prototype._open = function (name, cb) {
  process.nextTick(function () { cb(new SMBError(ntstatus.STATUS_NOT_IMPLEMENTED)); });
};
...
Tree.prototype.list = function (pattern, cb) {
  this._list(this.unicodeNormalize(pattern), cb);
};
 ...
 * @param {File[]} cb.files array of matching files
 */
Tree.prototype._list = function (pattern, cb) {
  process.nextTick(function () { cb(new SMBError(ntstatus.STATUS_NOT_IMPLEMENTED)); });
};
...
Tree.prototype.createFile = function (name, cb) {
  this._createFile(this.unicodeNormalize(name), cb);
};

/**
 * Private implementation of createFile method. Should be overridden by child classes.
 *
 * @param {String} name file name
 * @param {Function} cb callback called on completion
 * @param {SMBError} cb.error error (non-null if an error occurred)
 * @param {File} cb.file created file
 */
Tree.prototype._createFile = function (name, cb) {
  process.nextTick(function () { cb(new SMBError(ntstatus.STATUS_NOT_IMPLEMENTED)); });
};
...
Tree.prototype.createDirectory = function (name, cb) {
  this._createDirectory(this.unicodeNormalize(name), cb);
};

/**
 * Private implementation of createDirectory method. Should be overridden by child classes.
 *
 * @param {String} name directory name
 * @param {Function} cb callback called on completion
 * @param {SMBError} cb.error error (non-null if an error occurred)
 * @param {File} cb.file created directory
 */
Tree.prototype._createDirectory = function (name, cb) {
  process.nextTick(function () { cb(new SMBError(ntstatus.STATUS_NOT_IMPLEMENTED)); });
};
...
Tree.prototype.delete = function (name, cb) {
  this._delete(this.unicodeNormalize(name), cb);
};

/**
 * Private implementation of delete method. Should be overridden by child classes.
 *
 * @param {String} name file name
 * @param {Function} cb callback called on completion
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
Tree.prototype._delete = function (name, cb) {
  process.nextTick(function () { cb(new SMBError(ntstatus.STATUS_NOT_IMPLEMENTED)); });
};
...
Tree.prototype.deleteDirectory = function (name, cb) {
  this._deleteDirectory(this.unicodeNormalize(name), cb);
};

/**
 * Private implementation of deleteDirectory method. Should be overridden by child classes.
 *
 * @param {String} name directory name
 * @param {Function} cb callback called on completion
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
Tree.prototype._deleteDirectory = function (name, cb) {
  process.nextTick(function () { cb(new SMBError(ntstatus.STATUS_NOT_IMPLEMENTED)); });
};
...
Tree.prototype.rename = function (oldName, newName, cb) {
  this._rename(this.unicodeNormalize(oldName), this.unicodeNormalize(newName), cb);
};

/**
 * Private implementation of rename method. Should be overridden by child classes.
 *
 * @param {String} oldName old name
 * @param {String} newName new name
 * @param {Function} cb callback called on completion
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
Tree.prototype._rename = function (oldName, newName, cb) {
  process.nextTick(function () { cb(new SMBError(ntstatus.STATUS_NOT_IMPLEMENTED)); });
};
...
Tree.prototype.refresh = function (folderPath, deep, cb) {
  this._refresh(this.unicodeNormalize(folderPath), deep, cb);
};

/**
 * Private implementation of refresh method. Should be overridden by child classes.
 *
 * @param {String} folderPath
 * @param {Boolean} deep
 * @param {Function} cb callback called on completion
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
Tree.prototype._refresh = function (folderPath, deep, cb) {
  process.nextTick(function () { cb(new SMBError(ntstatus.STATUS_NOT_IMPLEMENTED)); });
};
...
Tree.prototype.traverseDirectory = function (startPath, dirCb, fileCb, cb) {
  startPath = this.unicodeNormalize(startPath);
  var tree = this;
